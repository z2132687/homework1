...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //
        val ed_name = findViewById<EditText>(R.id.ed_name)//宣告+綁定
        ...
        val tv_comra = findViewById<TextView>(R.id.tv_comra)
        btn_mora.setOnClickListener {
        //
            if (ed_name.length() < 1) {
                tv_text.text = "請輸入玩家姓名"//直觀
                return@setOnClickListener
            }
            //文字名稱作為玩家姓名
            val playerName = ed_name.text
            //產生介於 0~1 之間不含 1 的小數，並乘以 3 作為電腦的出拳
